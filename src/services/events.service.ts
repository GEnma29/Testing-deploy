...
export const eventsFetcher = (url: string) =>
  EventsInstance.get(url).then((res) => res.data);
...
// create event
export const createEvent = (url: string, { arg }: { arg: FormData }) =>
...
export const updateEvent = (url: string, { arg }: { arg: FormData }) =>
  fetch(`${baseURL}/core/api/${url}`, {
    method: 'PUT',
    body: arg,
    headers: {
...
// testing with axios
export const updateEventAxios = (eventId: string, data: FormData) =>
  EventsInstance.post(`/${eventId}`, data);
