    ...
    return (
        <div className='flex w-full items-center justify-between'>
            ...
            <div className='flex font-bold text-primary-300'>
                <div onClick={isDesktopView ? () => { } : actionLeft} className='flex cursor-pointer items-center '>
                    <ArrowBack className='lg:hidden' />
                    <h1 className='text-xs lg:text-2xl '>{title}</h1>
                </div>
            </div>
            <div className='flex'>
                <div onClick={actionRight} className={textRight ? 'flex items-center cursor-pointer' : 'hidden'}>
                    <p className=' text-xs mr-2  lg:text-lg text-black-300 '>{textRight}</p>
