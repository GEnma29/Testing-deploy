...
const LoginForm: React.FC = () => {
  ...
  const onSubmit = async (data: FormLogin) => {
    setLoading(true);
    const { access_token } = await authService({
      email: data.email,
    ...
    saveUser({
      ...
      last_name,
      email,
      role: { public: roles.public, funkart: roles.funkart },
    });
    // navigate
    setLoading(false);
  ...
  return (
    <div className="flex flex-col justify-center items-center w-full  lg:w-80  f-full">
      <h1 className="flex mt-4 text-3xl font-normal text-primary-300 w-full justify-center items-center lg:mt-16 lg:items-start lg:justify-start ">
        {' '}
        Inicia sesión
      </h1>
      <Form
      ...
      >
        ...
        <div className="flex my-4 flex-row">
          <input
            ...
            name="remember"
            id="remember"
            className="h-4 w-4 rounded border-gray-300 text-primary-300 focus:ring-primary-500 checked:bg-primary-300"
          />
        ...
        <div className="flex my-2">
          <p>
            ...
            <a className="underline	" onClick={goToRecoveryPassword} href={''}>
              Clickea aquí
            </a>
          </p>
        </div>
