...
const TablePendingPayment = () => {
    ...
    // url
    const key = useMemo(() => {
        return `/payments?sort=created_at&limit=${limit}&page=${page}&start_date=${dayjs(date?.from).format()}&end_date=${dayjs(date?.to).format()}&extra_1=${event}`;
    ...
    return (
        <TableLayout
            filters={
                <div className="flex align-items justify-between w-full">
                    ...
                    <div className="flex mx-3 flex-col w-32">
                        <p className="flex text-blod">Descarga Excel</p>
                        <DownloadData />
                    </div>
            ...
            } table={<Table isLoading={isLoading} data={dataTable} columns={columns} />}
            pagination={
                <PaginationTable
                    totalPages={10}
                    currentPage={page}
